...
<tr><td style="padding:32px 28px"><p style="margin:0 0 12px;color:#0f766e;font-size:12px;font-weight:bold;letter-spacing:1px">${escape(data.eventName)}</p>
...
${url ? `<table role="presentation" cellspacing="0" cellpadding="0" style="margin:28px 0"><tr><td bgcolor="#0f766e" style="border-radius:12px"><a href="${escape(url)}" style="display:inline-block;padding:16px 24px;color:#fff;font-size:15px;font-weight:bold;text-decoration:none">${cta}</a></td></tr></table><p style="font-size:12px;line-height:1.6;color:#64748b">Ce lien est personnel : conservez-le et ne le partagez pas.<br>Si le bouton ne fonctionne pas :<br><a href="${escape(url)}" style="color:#0f766e;word-break:break-all">${escape(url)}</a></p>` : ""}
