...
export async function financeRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/api/finance/invoices/:id/transmit-super-pdp", async (request) => {
    ...
    const response = await fetch(`https://api.superpdp.tech/v1.beta/invoices?processing_rule=B2BInt&external_id=${encodeURIComponent(invoice.id)}`, { method: "POST", headers: { authorization: `Bearer ${decryptCredential(connection.accessToken)}`, "content-type": "application/xml", accept: "application/json" }, body: ubl });
